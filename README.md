...
这是 CalDAV calendar tools 的 Nix-native OpenClaw wrapper。

这个仓库不保存任何日历凭据，也不实现新的 CalDAV 协议客户端。插件包会通过 Nix
提供一个稳定的 `caldav-calendar` CLI，并把 `vdirsyncer` 和 `khal` 放进
...
- 底层工具：`vdirsyncer`、`khal`
- 作用：为 OpenClaw 提供 CalDAV 日历同步、查询、创建和编辑能力。
...
- `devShells.${system}.default`：带有 wrapper、`vdirsyncer` 和 `khal` 的本地开发环境。
...
OpenClaw runtime PATH，skill 会指导 AI 通过这个命令操作日历。
...
- `CALDAV_CALENDAR_CONFIG_DIR`：包含 `vdirsyncer/config` 和 `khal/config` 的配置目录。
...
第一项用于 nix-openclaw 渲染 typed `config.settings`。后两项用于日历同步状态、本地
`.ics` 文件和 `khal` cache。

## 使用方式

日常流程通常是先同步，再查询或修改，最后再次同步：
